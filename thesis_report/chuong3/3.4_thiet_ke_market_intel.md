# 3.4 Thiết kế Market Intelligence Dashboard

## 3.4.1 Vai trò trong hệ thống

Market Intelligence Dashboard (gọi tắt **Market Intel**) là đóng góp khoa học chính số 3 của đề tài — tầng trình bày dữ liệu thị trường tuyển dụng CNTT Việt Nam. Khác với CV Health Dashboard (trình bày trong [mục 3.7](./3.6_thiet_ke_kien_truc.md)) tập trung vào trạng thái cá nhân của người dùng, Market Intel cung cấp **bức tranh vĩ mô** giúp sinh viên đối chiếu CV của mình với mặt bằng chung của thị trường:
...
Trong kiến trúc tổng thể, Market Intel là một trong hai trụ chính của đề tài, bổ trợ cho CV Health (Multi-criteria Freshness):

```mermaid
flowchart LR
    CV[CV Health<br/>Multi-criteria Freshness] --- MI[Market<br/>Intelligence Dashboard]
    style MI fill:#6366f1,color:#fff
    style CV fill:#10b981,color:#fff
```

Market Intel **không cá nhân hóa** — mọi người dùng truy cập đều thấy cùng dữ liệu. Việc cá nhân hóa thuộc về CV Health Dashboard. Tuy nhiên hai dashboard chia sẻ chung pool dữ liệu JD và Skill Ontology, nên insight thị trường (top skill, salary distribution theo role) trực tiếp feed vào chiều Skill và Market Alignment của Freshness.
...
|---|---|---|
| **ITviec** | ~810 | JD chuyên IT, full description (100%), work_mode/seniority đầy đủ (≥88%) |
| **TopCV** | ~500 | JD CNTT, min_exp 100% từ listing badge, salary cao hơn (43% có giá trị) |

Tổng cộng ~1,300 JD sau cross-source dedup, đảm bảo các aggregation thống kê có ý nghĩa (most insight cần ≥30 mẫu/bucket để tránh nhiễu). Pipeline crawl daily qua cron `@reboot` + 09:00 để cập nhật snapshot, dùng `job_group_id` cho cross-source dedup (xem [mục 3.4.3](./3.3_thiet_ke_pipeline_crawl.md#343-deduplication--hai-tầng-jd_key-và-job_group_id)).

**Filter switch giữa các nguồn**. Dashboard cho phép user chọn ITviec / TopCV / cả hai để so sánh insight giữa hai nền tảng. Ví dụ: sinh viên có thể quan sát "skill nào hot trên ITviec nhưng không hot trên TopCV" → hiểu được phân khúc nhân lực mỗi nền tảng phục vụ.

## 3.4.3 Nguyên tắc thiết kế

Market Intel áp dụng các nguyên tắc dashboard của Few (2006) [[9]](../tai_lieu_tham_khao.md#ref-9) (đã trình bày trong [mục 2.3.3](../chuong2/2.2_market_intelligence.md#233-information-dashboard-design-cho-thị-trường-tuyển-dụng)), bổ sung thêm ba nguyên tắc riêng cho **analytics dashboard**:
...
[← 3.3 Pipeline crawl JD + cross-source dedup](./3.3_thiet_ke_pipeline_crawl.md) | [→ 3.5 Skill Extraction & Matching Pipeline](./3.5_thiet_ke_skill_extraction_pipeline.md)
