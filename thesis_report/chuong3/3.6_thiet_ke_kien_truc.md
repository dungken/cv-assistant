# 3.6 Thiết kế Kiến trúc Tổng thể và CV Health Dashboard
...
flowchart TB
    ...
    subgraph Services[Service Layer]
        ...
        Skill[Skill Service<br/>FastAPI<br/>port 5002<br/>★ Multi-criteria Freshness + Market Intel]
...
|---|---|---|
| `POST /cv/freshness` | Tính Freshness Score (chiều Skill v1) cho CV (input: skills + role + snapshot_date) | Đã triển khai (Tuần 10) |
| `POST /cv/freshness-multi` | Tính Multi-criteria Freshness 8 chiều (input: CVProfileInput đủ 8 dim field) | Đã triển khai (Tuần 11) |
| `GET /ontology/skill/{name}` | Trả node skill và metadata (category, aliases, REQUIRES) | Đã triển khai (Tuần 10) |
...
- `real_jds`: vector của JD đã crawl (mô tả trong [mục 3.4.4](./3.3_thiet_ke_pipeline_crawl.md)).
...
### Asynchronous — Background tasks & Human-in-the-loop

Luồng upload CV được thiết kế theo pattern **Human-in-the-loop** để giải quyết sai sót của mô hình NER:
...
sequenceDiagram
    ...
    FE->>GW: POST /api/cv/extract (upload CV)
    GW->>SK: POST /cv/extract
    SK->>NER: Extract entities
    NER-->>SK: list[skills]
    SK-->>GW: Trả về danh sách skills (chưa tính điểm)
    GW-->>FE: Hiển thị UI Review Skills
    
    Note over FE: User thêm/xóa skill bị thiếu
    
    FE->>GW: POST /api/cv/confirm {verified_skills}
    GW->>SK: POST /cv/confirm
    SK->>SK: Tính Freshness Score 8 chiều
    SK->>DB: INSERT cv_documents & skill_freshness_history
    SK-->>GW: 200 OK
    GW-->>FE: Chuyển sang CV Health Dashboard
```
...
## 3.6.7 Thiết kế CV Health Dashboard (deliverable end-user)

CV Health Dashboard là giao diện tích hợp end-user, trực quan hóa kết quả của 2 đóng góp khoa học chính (Multi-criteria Freshness, Skill alerts) cho sinh viên. Khác với Market Intel Dashboard (mục [3.5](./3.4_thiet_ke_market_intel.md)) cung cấp bức tranh vĩ mô không cá nhân hóa, CV Health Dashboard hiển thị **trạng thái CV riêng của từng user**.

### Nguyên tắc thiết kế UI

Dashboard áp dụng các nguyên tắc của Few (2006) [[9]](../tai_lieu_tham_khao.md#ref-9) (đã trình bày trong [mục 2.3.3](../chuong2/2.2_market_intelligence.md#233-information-dashboard-design-cho-thị-trường-tuyển-dụng)):
...
### Bố cục Dashboard và What-if Simulation

CV Health Dashboard được thiết kế không chỉ để xem (view) mà còn để **tương tác (interact)** thông qua tính năng **What-if Simulation**:
...
flowchart TB
    subgraph Top[Above the fold]
        ...
        A3[Opportunity Window<br/>Top JD phù hợp tuần này]
    end
    subgraph Simulation[What-if Simulation Mode]
        S1[Toggle: Bật chế度假 lập]
        S2[Kéo thả kỹ năng hot vào CV]
        S3[Update Radar Chart & Cơ hội việc làm realtime]
    end
...
- **Cross-pillar Navigation**: Từ bảng B2 (Top trending skills) được lấy từ Market Intel, người dùng có thể click "Thử áp dụng vào CV" để lập tức kích hoạt chế độ Simulation, đưa kỹ năng đó vào hồ sơ giả định và quan sát điểm số biến đổi.
...
- **SkillAlertsList**: Card có tên skill + status badge (Trending Up/Down/Emerging) + lý do ("Demand tăng 25% trong 4 tuần qua") + CTA "Xem thêm".
...
## 3.6.8 Tổng kết Chương 3

Chương 3 đã trình bày thiết kế chi tiết cho hệ thống CV Health Intelligence:

1. **Yêu cầu hệ thống** ([3.1](./3.1_phan_tich_yeu_cau.md)): 7 nhóm chức năng (FR-A đến FR-G), yêu cầu phi chức năng, 4 bài toán kỹ thuật mapping với 2 đóng góp + infrastructure + integration.

2. **Multi-criteria CV Freshness Framework** ([3.2](./3.2_thiet_ke_freshness_score.md)) — **đóng góp 1**: khung 8 chiều với công thức từng chiều + Weighted Sum Model + trọng số theo seniority + 5 tính chất + 3 phương pháp validate.

3. **Pipeline crawl JD + cross-source dedup** ([3.3](./3.3_thiet_ke_pipeline_crawl.md)): cron `@reboot` + daily 09:00 + stamp file, adapter pattern, 2 tầng dedup (`jd_key` URL-based + `job_group_id` strict company+title).

4. **Market Intelligence Dashboard** ([3.4](./3.4_thiet_ke_market_intel.md)) — **đóng góp 2**: 33 insight chia 5 nhóm, filter switch ITviec/TopCV/cả hai, 8 nguyên tắc dashboard (5 Few + 3 analytics).

5. **Skill Extraction & Matching Pipeline** ([3.5](./3.5_thiet_ke_skill_extraction_pipeline.md)) — **infrastructure**: NER mBERT 21 nhãn BIO + Ontology 460 entries 15 category + Cascade Matching 3 tầng (Exact/Ontology/SBERT).

6. **Kiến trúc tổng thể + CV Health Dashboard** (mục này): 6 microservices, schema PostgreSQL + ChromaDB, communication patterns (sync/async/scheduled), Docker Compose deployment, UI dashboard 8 chiều cho user.

Chương 4 sẽ trình bày kết quả thực nghiệm: data quality, validate Multi-criteria Framework, đánh giá đa tầng Skill Extraction Pipeline, Market Intel insights.

---

[← 3.5 Skill Extraction & Matching Pipeline](./3.5_thiet_ke_skill_extraction_pipeline.md) | [→ Chương 4: Xây dựng và Thực nghiệm](../chuong4/4.2_jd_crawler.md)
