# 3.5 Thiết kế Skill Extraction & Matching Pipeline

> **Lưu ý:** Mục này trình bày thiết kế cho **Bilingual Skill Extraction & Matching Pipeline** — thành phần infrastructure tiền xử lý CV/JD cho hai đóng góp khoa học chính của đề tài (Multi-criteria Freshness, Market Intel). Pipeline này không phải là đóng góp khoa học chính nhưng là nền tảng kỹ thuật cần thiết. Cơ sở lý thuyết của NER, Ontology, SBERT, Cascade Matching đã được trình bày trong [mục 2.5](../chuong2/2.4_pipeline_trich_xuat_ky_nang.md); mục này tập trung vào phần thiết kế cài đặt cụ thể.

## 3.5.1 Tổng quan kiến trúc pipeline
...
flowchart LR
    ...
    Out --> Out2[Market Intel Dashboard<br/>đóng góp 2]
```
...
## 3.5.2 NER mBERT fine-tune cho CV/JD song ngữ Việt-Anh

### Lựa chọn mô hình base

Đề tài chọn `bert-base-multilingual-cased` (mBERT) làm backbone NER với các lý do đã phân tích trong [mục 2.5.1](../chuong2/2.4_pipeline_trich_xuat_ky_nang.md#251-named-entity-recognition-và-mô-hình-bert-đa-ngôn-ngữ-mbert) — phù hợp nhất với CV/JD CNTT Việt Nam có hiện tượng code-switching Việt-Anh. mBERT có 178M parameters, hỗ trợ 104 ngôn ngữ với vocabulary chung 119.547 WordPiece tokens.
...
## 3.5.3 Skill Ontology IT 460 entries (REQUIRES, LEADS_TO, RELATED_TO, PART_OF)
...
4. **Metadata enrichment**: gán `category`, `domain`, `aliases` cho mỗi canonical — phục vụ chiều Skill và Market Alignment của Multi-criteria Freshness ([mục 3.2](./3.2_thiet_ke_freshness_score.md)).
5. **Continuous learning**: log các skill được trích xuất nhưng không match (`unmatched_skills`), review mỗi 2 tuần để bổ sung.

## 3.5.4 Cascade Matching: Exact → Ontology → Sentence-BERT cosine
...
[← 3.4 Thiết kế Market Intelligence Dashboard](./3.4_thiet_ke_market_intel.md) | [→ 3.6 Thiết kế kiến trúc tổng thể + CV Health Dashboard](./3.6_thiet_ke_kien_truc.md)
