...
Mục này trình bày thiết kế chi tiết cho Learning Path Optimizer — đóng góp khoa học chính số 2 của đề tài. Bao gồm: hình thức hóa bài toán cho bối cảnh CV thật, kiến trúc các thuật toán đã cài đặt, và thiết kế bộ benchmark 50 test cases để so sánh ba thuật toán. Cơ sở toán học của 3 thuật toán đã được trình bày trong [mục 2.2](../chuong2/2.2_skill_graph_learning_path.md); mục này tập trung vào phần thiết kế cài đặt và đánh giá.

## 3.3.1 Định nghĩa bài toán trong bối cảnh CV thực tế

Bài toán đã được hình thức hóa tổng quát trong [mục 2.2.2](../chuong2/2.2_skill_graph_learning_path.md#222-hình-thức-hóa-bài-toán-learning-path-optimization). Khi đưa vào hệ thống thực tế, một số yếu tố cần được làm rõ thêm:
...
| Nhóm 5 — Test thực tế | 10 | CV mẫu của sinh viên CNTT năm 3-4 + JD thực từ snapshot ITviec/TopCV |
...
## 3.3.6 Kết quả benchmark sơ bộ (Tuần 13)

Sau khi cài đặt cả ba thuật toán theo thiết kế ở các mục 3.3.2–3.3.3 và chạy trên bộ 50 test case mô tả ở 3.3.4, kết quả benchmark sơ bộ như sau (chi tiết kỹ thuật và phân tích đầy đủ sẽ được trình bày trong Chương 4).

**Bảng 3.3: Kết quả tổng hợp 3 thuật toán trên 50 test cases**

| Thuật toán | Optimality (mean / min) | Runtime (mean / max) | Prereq compliance | Within budget |
|---|---|---|---|---|
| Greedy   | **0.99 / 0.50** | **0.19 ms / 0.90 ms** | 100% | 100% |
| Dijkstra | 1.00 / 1.00 | 10.59 ms / 174 ms | 100% | 100% |
| DP       | 1.00 / 1.00 | 240.88 ms / 2.33 s | 100% | 100% |

**Quan sát chính.**

- **Greedy đạt 0.99 optimality** trung bình, vượt kỳ vọng 0.8 từ thiết kế. Chỉ một test case duy nhất ở nhóm `real` bị tụt xuống opt_ratio = 0.5 (Greedy tìm được 1 JD, DP tìm được 2). Trong các trường hợp còn lại Greedy bằng hoặc gần bằng DP.
- **Greedy nhanh hơn Dijkstra ~55× và nhanh hơn DP ~1300×** — phù hợp làm default cho real-time API như đã đề xuất ở 3.3.2.
- **Cả 3 thuật toán đạt 100%** trên hai metric an toàn (prereq compliance, within budget) nhờ logic `expand_with_prereqs` được dùng chung.
- **Greedy ổn định** (metric 3 ở 3.3.4): cùng một test case shuffle ngẫu nhiên thứ tự JD và `S_user` qua 5 random seed cho ra cùng JD count → đáp ứng yêu cầu "stability".
- **DP runtime mean 240 ms** thoạt nhìn cao, nhưng tập trung ở nhóm `wide_budget` (~1.2 s mean) do số candidate skill lên đến 18, dẫn đến không gian $2^{18} \approx 2.6 \times 10^5$ subset cần duyệt. Vẫn dưới ngưỡng 60 s mà thiết kế đặt ra.

**Lưu ý kỹ thuật về DP.** Trong quá trình cài đặt, đề tài phát hiện công thức DP bitmask "ngây thơ" theo §2.2.4 không cho lời giải tối ưu cho mọi trường hợp khi hàm mục tiêu `JD_unlocked` không tách rời (non-separable) theo skill. Cụ thể: nếu một bước trung gian chưa unlock JD nào (gain = 0) nhưng là tiền đề cho một combination unlock được JD ở bước sau, DP có thể loại bỏ subset đó vì cell `(i, b)` chỉ lưu một lời giải duy nhất. Đề tài giải quyết bằng cách: với $|V_{\text{cand}}| \leq 20$ chuyển sang duyệt toàn bộ $2^n$ subset (chính xác, $n=18$ vẫn chạy ~2.3 s); với $20 < |V_{\text{cand}}| \leq 30$ giữ DP bitmask kèm tie-break ưu tiên subset lớn và post-prune dư thừa. Không có test case nào trong 50 cases vượt 18 candidates nên DP chạy ở chế độ chính xác cho toàn bộ benchmark.

Báo cáo benchmark đầy đủ (chi tiết per-group, runtime distribution, edge cases) sẽ được trình bày trong Chương 4.

---

[← 3.2 Multi-criteria CV Freshness Framework](./3.2_thiet_ke_freshness_score.md) | [→ 3.4 Thiết kế pipeline crawl JD + cross-source dedup](./3.4_thiet_ke_pipeline_crawl.md)
