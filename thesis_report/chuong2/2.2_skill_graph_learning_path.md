...
**Khái niệm.** Skill Graph là một dạng cụ thể của Knowledge Graph, trong đó các nút (nodes) là kỹ năng và các cạnh (edges) là quan hệ có nhãn giữa các kỹ năng. Hình thức hóa: skill graph là bộ ba $G = (V, E, L)$, trong đó $V$ là tập kỹ năng, $E \subseteq V \times R \times V$ là tập cạnh có nhãn quan hệ, và $L: V \cup E \to \text{Attributes}$ là hàm gán metadata cho nút và cạnh. Cấu trúc đồ thị (thay vì cây) cho phép biểu diễn nhiều loại quan hệ song song và truy vấn theo path nhiều bước.

**Bốn loại quan hệ cốt lõi.** Đề tài định nghĩa skill graph với bốn loại quan hệ:

- **REQUIRES** (yêu cầu tiên quyết). Nếu $A \xrightarrow{\text{REQUIRES}} B$, để học/sử dụng A một cách hiệu quả, người học cần biết B trước. Ví dụ: `React REQUIRES JavaScript`, `Django REQUIRES Python`, `Kubernetes REQUIRES Docker`. Quan hệ này có tính bắc cầu hợp lý nhưng trong implementation không suy diễn tự động để tránh phình quan hệ.
- **LEADS_TO** (dẫn đến). Nếu $A \xrightarrow{\text{LEADS_TO}} B$, người học A thường có xu hướng học B tiếp theo. Ví dụ: `JavaScript LEADS_TO React`, `Python LEADS_TO Django`. Mang tính gợi ý/xác suất, không bắt buộc như REQUIRES.
- **RELATED_TO** (liên quan/thay thế). Nếu $A \xleftrightarrow{\text{RELATED_TO}} B$, hai kỹ năng có thể thay thế cho nhau hoặc cùng ecosystem. Ví dụ: `React RELATED_TO Vue.js`, `PostgreSQL RELATED_TO MySQL`. Quan hệ đối xứng.
- **PART_OF** (thành phần). Nếu $A \xrightarrow{\text{PART_OF}} B$, A là một thành phần của B. Ví dụ: `React PART_OF Frontend Development`, `PostgreSQL PART_OF Database`. Cho phép phân nhóm kỹ năng theo domain.

**Metadata trên nút.** Mỗi kỹ năng (nút) có các trường metadata bổ sung:

- `domain`: nhóm chính (frontend, backend, data, devops, ai_ml, ...).
- `category`: phân nhóm con (programming_language, framework, database, ...).
- `learning_cost`: số tuần học ước tính từ trạng thái cơ bản đến mức làm việc được.
- `aliases`: các biến thể tên gọi (ví dụ "ReactJS" ↔ "React.js" ↔ "React").
...
**So sánh với các bộ ontology kỹ năng tiêu biểu.** Hai bộ ontology được công bố rộng rãi nhất là ESCO [[14]](../tai_lieu_tham_khao.md#ref-14) (~13.890 kỹ năng, 27 ngôn ngữ châu Âu, không có tiếng Việt) và O*NET [[15]](../tai_lieu_tham_khao.md#ref-15) (~16.000 kỹ năng cho ~1.110 nghề nghiệp tại Mỹ, tiếng Anh duy nhất). Cả hai đều không hỗ trợ tiếng Việt và không phản ánh đầy đủ thực trạng thị trường CNTT Việt Nam (alias đặc thù, công nghệ cập nhật sau 2022). Đề tài tự xây dựng skill graph **460 nodes thuộc 15 category, 418 cạnh** cho domain CNTT Việt Nam — đủ phủ các nhánh phổ biến (Backend, Frontend, Mobile, Data, DevOps, AI/ML) và là sự cân bằng giữa độ phủ và tính khả thi.

## 2.2.2 Hình thức hóa bài toán Learning Path Optimization
...
## 2.2.3 Thuật toán Greedy — ROI-based
...
## 2.2.4 Thuật toán Dijkstra trên Skill Graph
...
## 2.2.5 Dynamic Programming — Lời giải tối ưu
...
[← 2.1 MCDM và đánh giá CV đa tiêu chí](./2.1_mcdm_danh_gia_cv.md) | [→ 2.3 Market Intelligence](./2.3_market_intelligence.md)
