...
- **Skill Service** (port 5004): cung cấp API cho Multi-criteria Freshness Framework, Learning Path Optimizer, Market Intelligence Dashboard.
...
- **`bert-base-multilingual-cased` (mBERT)**: backbone cho NER song ngữ Việt-Anh, fine-tune cho 21 nhãn BIO domain CV/JD CNTT (xem [mục 2.5.1](./2.5_pipeline_trich_xuat_ky_nang.md#251-named-entity-recognition-và-mô-hình-bert-đa-ngôn-ngữ-mbert)).
...
**pytest.** Framework testing cho Python. Sử dụng cho: unit test các hàm thuần (Freshness formula, ROI calculation, cross-source dedup), integration test cho API endpoints, benchmark test cho 3 thuật toán Learning Path Optimization.

## 2.6.5 Tổng kết stack công nghệ
...
Sơ đồ minh họa 4 tầng kiến trúc: Frontend, API Gateway, Microservices và Data Layer. Crawler chạy theo cron host (với fallback `@reboot`) kết nối với nguồn ngoài (ITviec, TopCV) qua AJAX endpoint. Skill Service là service trung tâm chứa logic cho ba đóng góp khoa học chính (Multi-criteria Freshness, Learning Path Optimizer, Market Intelligence Dashboard).

---

[← 2.5 Pipeline tiền xử lý CV/JD](./2.5_pipeline_trich_xuat_ky_nang.md) | [→ Chương 3: Phân tích và Thiết kế hệ thống](../chuong3/3.1_phan_tich_yeu_cau.md)
