...
const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        <>
            <aside className={cn(
                "sidebar-transition bg-secondary flex flex-col h-full relative z-50 border-r border-white/5",
                isOpen ? "w-[300px] overflow-visible" : "w-[68px] overflow-hidden"
            )}>
                <div className={cn("h-16 flex items-center px-[10px] shrink-0 transition-colors duration-200 relative overflow-hidden")}>
                    <Button
                        variant="ghost"
                        size="icon"
                        onClick={onToggle}
                        className="w-12 h-12 hover:bg-surface-hover rounded-full flex items-center justify-center transition-colors duration-200 shrink-0"
                    >
                        <Menu className="w-5 h-5" strokeWidth={1.5} />
                    </Button>
                </div>

                <div ref={scrollContainerRef} className={cn("flex-1 flex flex-col space-y-1 overflow-y-auto floating-scrollbar scroll-smooth pt-2", !isOpen && "no-scrollbar")}>
                    ...
                    <div className="space-y-1">
                        <Button
                            ...
                            variant="ghost"
                            className={cn(
                                "h-10 flex items-center !justify-start group transition-colors duration-200 overflow-hidden mx-[10px] px-[14px] border",
                        ...
                        <Button
                            ...
                            className={cn(
                                "h-10 flex items-center !justify-start group transition-colors duration-200 overflow-hidden mx-[10px] px-[14px] border",
                                isOpen ? "w-[calc(100%-20px)] rounded-xl border-transparent" : "w-12 rounded-full shadow-lg shadow-black/5 bg-surface/50 border-white/5",
                                location.pathname === '/market-intel' && "bg-surface-hover text-white border-white/10 shadow-black/20"
                            )}
                        >
                            <TrendingUp className={cn("w-5 h-5 transition-transform group-hover:scale-110 shrink-0", location.pathname === '/market-intel' ? "text-white" : "text-text-secondary")} strokeWidth={1.5} />
                            <span className={cn("sidebar-content-slide font-bold text-[13px] text-left ml-4 transition-all", isOpen ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4", location.pathname === '/market-intel' ? "text-white" : "text-text-primary")}>
                                Market Intel
                            </span>
                        </Button>
                ...
                <div className="mt-auto pb-4 space-y-1 shrink-0">
                    <Button onClick={onOpenSettings} variant="ghost" className={cn("group transition-colors duration-200 flex items-center !justify-start overflow-hidden mx-[10px] px-[14px]", isOpen ? "w-[calc(100%-20px)] h-11 rounded-xl" : "w-12 h-11 rounded-full hover:bg-surface-hover")}>
                    ...
                    <div className="relative px-0 mt-2">
                         <div className={cn("flex items-center transition-colors duration-200 h-16", isOpen ? "justify-between" : "justify-start")}>
